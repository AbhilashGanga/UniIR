...
class MBEIRDatasetBase(Dataset):
    ...
    def _load_and_preprocess_image(self, query_img_path):
        ...
        #image = self.img_preprocess_fn(images=image, return_tensors="pt")
        return image
...
class MBEIRCollatorBase(object):
    ...
    def __init__(self, tokenizer: Callable[[List[str]], Any], image_size: Union[tuple, int]):
        ...
        self.padded_image = Image.new('RGB', (self.H, self.W), 'black')#torch.zeros((3, self.H, self.W))
...
class MBEIRMainCollator(MBEIRCollatorBase):
    ...
    def __call__(self, batch):
        ...
        elif hasattr(processed_batch["txt_batched"], "input_ids") and type(processed_batch["txt_batched"]["input_ids"]) == list:
            bs = len(processed_batch["txt_batched"]["input_ids"])
        else:
            bs = len(processed_batch["txt_batched"])
        if hasattr(processed_batch["image_batched"], "input_ids") and type(processed_batch["image_batched"]["input_ids"]) != list:
            bs = processed_batch["image_batched"]["input_ids"].size(0)
        elif hasattr(processed_batch["image_batched"], "input_ids") and type(processed_batch["image_batched"]["input_ids"]) == list:
            bs = len(processed_batch["image_batched"]["input_ids"])
        else:
             bs = len(processed_batch["image_batched"])
