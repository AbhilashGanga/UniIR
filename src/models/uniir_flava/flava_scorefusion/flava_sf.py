...
import numpy as np
import requests
from transformers import AutoProcessor, AutoImageProcessor, AutoTokenizer, FlavaModel



class FlavaScoreFusion(nn.Module):
    def __init__(self, model_name="ViT-B/32", device="cuda", jit=False, download_root=None, config=None):
        ...
        self.img_preprocess_fn = AutoImageProcessor.from_pretrained("facebook/flava-full")
    ...
    def encode_multimodal_input(self, txt_tensor, img_tensor, txt_mask, img_mask):
        ...
        txt_tensor["pixel_values"] = img_tensor
        # for e in txt_tensor:
        #     if not isinstance(txt_tensor, torch.Tensor):
        #         print(e)
        #         print(txt_tensor[e])
        #         txt_tensor[e] = torch.as_tensor(np.asarray(txt_tensor[e]))
        # print(txt_tensor)
        outputs = self.model(**txt_tensor)
        

        image_embeddings = outputs.image_embeddings
        text_embeddings = outputs.text_embeddings
        multimodal_embeddings = outputs.multimodal_embeddings
        txt_emb = text_embeddings[:,0,:].squeeze(1) * txt_mask.unsqueeze(-1)
        img_emb = image_embeddings[:,0,:].squeeze(1) * img_mask.unsqueeze(-1)
        return multimodal_embeddings[:,0,:].squeeze(1)  # shape: [batch_size, embed_dim]
