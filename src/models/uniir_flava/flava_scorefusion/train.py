...
from models.uniir_flava.engine import train_one_epoch, eval_engine
from models.uniir_flava import utils
from flava_sf import FlavaScoreFusion
...
def main(config):
    ...
    #     config=config,
    # )
    model = FlavaScoreFusion(model_name = model_config.clip_vision_model_name,config=config)
    ...
    optimizer = create_optimizer(gain_or_bias_params, rest_params,frozen_params,config)
    ...
    img_preprocess_fn = AutoImageProcessor.from_pretrained("facebook/flava-full")
