...
from transformers import AutoProcessor, AutoImageProcessor, AutoTokenizer, AlignModel

class ALIGNScoreFusion(nn.Module):
    def __init__(self, model_name, device="cuda", jit=False, download_root=None, config=None):
        super().__init__()

        # Load pre-trained CLIP model
        self.align_model = AlignModel.from_pretrained("kakaobrain/align-base")
        self.img_preprocess_fn = AutoProcessor.from_pretrained("kakaobrain/align-base")
    ...
    def encode_text(self, text_tensor):
        anss = self.align_model.get_text_features(**text_tensor)
        return anss

    def encode_image(self, image_tensor):
        anss = self.align_model.get_image_features(**image_tensor)
        return anss
    ...
    def get_logit_scale(self):
        return 1/self.align_model.temperature
