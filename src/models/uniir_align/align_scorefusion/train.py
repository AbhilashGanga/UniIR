...
from data.mbeir_data_utils_align import (
    build_mbeir_dataset_from_config,
    DatasetType,
    build_distributed_sampler_list,
    build_dataloader_list,
)
from models.uniir_align.engine import train_one_epoch, eval_engine
from models.uniir_align import utils
from align_sf import ALIGNScoreFusion
...
def main(config):
    ...
    print("Creating ALIGN-SF model...")
    model_config = config.model
    pretrained_align_model_dir = os.path.join(config.uniir_dir, model_config.pretrained_align_model_dir)
    logger.info(f"Downloading ALIGN model to {pretrained_align_model_dir}...")
    model = ALIGNScoreFusion(
        model_name=model_config.align_vision_model_name,
        download_root=pretrained_align_model_dir,
        config=config,
    )
    model.float()  # The origial ALIGN was in fp16 so we need to convert it to fp32
    ...
    if ckpt_config.resume_training:
        ...
        logger.info(f"loading ALIGNScoreFusion checkpoint from {checkpoint_path}")
